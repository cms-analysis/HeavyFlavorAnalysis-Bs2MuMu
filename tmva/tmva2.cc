...
tmva2::tmva2() {
  ...
  // -- 2012 change lumi scale!!!
	fInputFiles.sname = "/shome/lazo-flores/scratch/v12-2e33-BdToPiMuNu.root"; 
	//	fInputFiles.sname = "/shome/lazo-flores/scratch/v12-cms-BdToMuMu.root"; 
//	fInputFiles.sname = "/shome/lazo-flores/scratch/v12-cms-BdToMuMu.root"; 
//	fInputFiles.sname = "/shome/lazo-flores/scratch/v12-2e33-BdToKK.root"; 
//	fInputFiles.sname = "/shome/lazo-flores/scratch/v12-2e33-BdToPiPi.root"; 
//	fInputFiles.sname = "/shome/lazo-flores/scratch/v12-2e33-BdToKPi.root"; 
	fInputFiles.dname = "/shome/lazo-flores/scratch/v12-2012-data-bdt.root";

//  fInputFiles.sname = "/shome/lazo-flores/scratch/v12-mctruth-cms-BsToMuMu.root"; 
//  fInputFiles.dname = "/shome/lazo-flores/scratch/v12-2012-data-bdt.root";
...
// ----------------------------------------------------------------------
void tmva2::setupTree(TTree *t, RedTreeData &b) {
...
// ----------------------------------------------------------------------
void tmva2::train(string oname, string filename) {
...
// ----------------------------------------------------------------------
void tmva2::apply(const char *fname) {
...
// ----------------------------------------------------------------------
void tmva2::reAnalyze(int imin, int imax) {
...
// ----------------------------------------------------------------------
void tmva2::analyze(const char *fname) {
...
//	  cout << "*** NEW ===> w8 = " << w8 << ", sCnt = " << sCnt << ", sCnt0 = " << sCnt0 << ", sCnt1 = " << sCnt1 << ", sCnt2 = " << sCnt2 << endl;
	  double swlow = 4.90; double swhi = 5.30;
//	  double s = sm->Integral(sm->FindBin(5.30), sm->FindBin(5.45));
	  double s = sm->Integral(sm->FindBin(swlow), sm->FindBin(swhi));
    double d = dm->Integral(0, dm->GetNbinsX());
    double seff = s/sCnt;
//	  double bsimple = d*(5.45-5.30)/(5.9-4.9-0.25);
	  double bsimple = d*(swhi-swlow)/(5.9-4.9-0.25);
    double deff = bsimple/(dLostEvents + dCnt);

    double dhi = dm->Integral(dm->FindBin(5.45), dm->GetNbinsX());
    double pbg = 0.07*s;
//    double b = dhi*(5.45-5.30)/(5.9-5.45);
	  double b = dhi*(swhi-swlow)/(5.9-5.45);
...
//    double s0 = sm0->Integral(sm0->FindBin(5.30), sm0->FindBin(5.45));
	  double s0 = sm0->Integral(sm0->FindBin(swlow), sm0->FindBin(swhi));
    double d0 = dm0->Integral(0, dm0->GetNbinsX());
//    double b0 = d0*(5.45-5.30)/(5.9-4.9-0.25);
	  double b0 = d0*(swhi-swlow)/(5.9-4.9-0.25);
...
//    double s1 = sm1->Integral(sm1->FindBin(5.30), sm1->FindBin(5.45));
	  double s1 = sm1->Integral(sm1->FindBin(swlow), sm1->FindBin(swhi));
    double d1 = dm1->Integral(1, dm1->GetNbinsX());
//    double b1 = d1*(5.45-5.30)/(5.9-4.9-0.25);
	  double b1 = d1*(swhi-swlow)/(5.9-4.9-0.25);
...
//    double s2 = sm2->Integral(sm2->FindBin(5.30), sm2->FindBin(5.45));
	  double s2 = sm2->Integral(sm2->FindBin(swlow), sm2->FindBin(swhi));
    double d2 = dm2->Integral(1, dm2->GetNbinsX());
//    double b2 = d2*(5.45-5.30)/(5.9-4.9-0.25);
	  double b2 = d2*(swhi-swlow)/(5.9-4.9-0.25);
...
// ----------------------------------------------------------------------
void tmva2::mvas(string fname) { //, HistType htype, Bool_t useTMVAStyle ) {
...
// ----------------------------------------------------------------------
TH1D* tmva2::getRanking(string fname, string prefix, std::string type) {
...
// ----------------------------------------------------------------------
void tmva2::cleanup(string fname) {
...
// ----------------------------------------------------------------------
void tmva2::makeAll(int offset, string filename, int clean) {
...
// ----------------------------------------------------------------------
void tmva2::make(int offset, string filename, int evt, int clean) {
...
// ----------------------------------------------------------------------
void tmva2::createInputFile(string filename) {
...
//	TCut massbg  = "!(5.2<m&&m<5.45)";
  ...
  for (int j = 0; j < 3; ++j) {
    ...
    for (int i = 0; i < nchan; ++i) {
	  ...
	  copyCuts = sgcut + masscut + chanCut + typeCut;
...
// ----------------------------------------------------------------------
void tmva2::writeOut(TFile *f, TH1 *h) {
...
// ----------------------------------------------------------------------
void tmva2::redrawStats(double x, double y, const char *newname, int color) {
...
// ----------------------------------------------------------------------
void tmva2::newLegend(double x1, double y1, double x2, double y2, string title) {
  if (legg) delete legg;
...
// ----------------------------------------------------------------------
void tmva2::toyRuns(string ifilename, int nruns) {
  string oname; 
  int offset(100);
  int seed(0); 
...
// ----------------------------------------------------------------------
void tmva2::createToyData(string ifilename, string ofilename, int seed, int nsg, int nbg) {
  delete gRandom; 
...
// ----------------------------------------------------------------------
void tmva2::trainOnToyData(string iname, string oname) {
...
// ----------------------------------------------------------------------
TTree* tmva2::createTree(struct readerData &rd) {
