...
void csm_channel_model::add_template(const TH1Input& template_hist, //Poisson or non-Poisson histogram
									 Double_t sf,        //scale factor to multiply template by to compare w/ data 
														 //(e.g., (data_lum/MC_lum) for a MC Poisson histogram
									 Int_t nnp,          // number of nuisance parameters (Gaussian of unit width)
									 char* npname[],     // nuisance parameter names 
									 Double_t *nps_low,  // fractional uncertainty on sf due to each nuisance parameter -- low side
									 Double_t *nps_high, // fractional uncertainty on sf due to each nuisance parameter -- high side
														 // typically nps_low and nps_high are input with opposite signs 
														 // -- if oppositevariations of the nuisance parameter create opposite   
														 // changes in sf. The signis retained in the calculation in case both  
														 // variations of a nuisance parameter shift the normalization in the same way
														 // (either both + or both -)
									 const TH1InputList& lowshape, // array of low hisogram shapes, one for each nuisance param 
																   // (null if no shape error)
									 Double_t *lowsigma,  // number of sigma low for each nuisance parameter shape variation
									 const TH1InputList& highshape,   // array of high histogram shapes, one for each nuisance 
																	  // param (null if no shape error)
									 Double_t *highsigma, // number of sigma high for each shape variation
									 Int_t pflag, // Poisson flag: 1 if Poisson, 0 if not, 2 if Gaussian error from the histo contents
									 Int_t sflag) // Scale flag: 1 if signal, 0 if background (for use with s95 calculator)
{
  int i;
  svstruct ses;
...
/*-------------------------------------------------------------------------*/
/* arguments:  
   beta: credibility level. L = 0.95 for 95% CL limits
