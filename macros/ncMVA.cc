...
void ncRunTraining(TTree *signalTree, double signalWeight, TTree *bkgTree, double bkgWeight, int channelIx, TCut channelCut, string mOptions)
{
	...
	TString bdtTitle(Form("BDT_%d",channelIx));
	...
	// book neural network
	factory->BookMethod(TMVA::Types::kMLP, methodTitle, mOptions);
...
void ncRunDefaultTraining(const char *mcFile, const char *dataFile)
{
	...
	string tmpfilename(Form("%s.root",tmp)); // add the ending
	TFile sigFile(mcFile);
	TFile bkgFile(dataFile);
	TFile tmpFile(tmpfilename.c_str(),"recreate");
	...
	TTree *sigTree = (TTree*)sigFile.Get("T");
