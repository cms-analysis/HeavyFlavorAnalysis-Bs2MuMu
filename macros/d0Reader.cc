...
d0Reader::d0Reader(TChain *tree, TString evtClassName) : treeReader01(tree, evtClassName),reduced_tree(NULL)
{
...
void d0Reader::eventProcessing()
{
	int j,nc;
	TAnaCand *pCand;
	TAnaTrack *piTrk, *kaTrk, *muTrk;
	...
	for (j=0; j<nc; j++) {
	    ...
	    fMass = pCand->fMass;
	    if(fCandidate==10010 && fabs(fMass-MDZERO)<0.3 ){
		  ...
		  if((pCand->fSig2-pCand->fSig1) != 1) {
		    cout <<  "Error detected for 10010: nTracks="<<pCand->fSig2-pCand->fSig1<<endl; 
		    continue;
		  }
		  kaTrk = fpEvt->getSigTrack(pCand->fSig1);
		  piTrk = fpEvt->getSigTrack(pCand->fSig2);


		  fdeltaR   = fMomentum.DeltaR(kaTrk->fPlab);
		  double dR =  fMomentum.DeltaR(piTrk->fPlab);
		  fdeltaR = dR<fdeltaR ? dR : fdeltaR;

		  fdeltaRMax   = fMomentum.DeltaR(kaTrk->fPlab);
		  dR =  fMomentum.DeltaR(piTrk->fPlab);
		  fdeltaRMax = dR>fdeltaRMax ? dR : fdeltaRMax;

		  ptmin=kaTrk->fPlab.Perp();
		  etamax=fabs(kaTrk->fPlab.Eta());

		  double pt_tmp=piTrk->fPlab.Perp();
		  double eta_tmp=fabs(piTrk->fPlab.Eta());
	     ...
	     } /* else if (fCandidate==20010 && fabs(fMass-MB_0)<0.5){
	          dppt= dpm= dpdxy= dpdxyE= dpd3d= dpd3dE= -9.;
	          bpt= bm= bdxy= bdxyE= bd3d= bd3dE= -9.;
	          fdistPV= fdistDp= fdeltaR= fdeltaRMu= fdeltaRMax =-9.;
		  chi2vtx= dalpha= -9.;
		  ...
		  pi2TrkCHANGE = fpEvt->getSigTrack(pCand->fSig1+2);
		  muTrk = fpEvt->getSigTrack(pCand->fSig2);

		  fdeltaR   = fMomentum.DeltaR(kaTrk->fPlab);
		  double dR =  fMomentum.DeltaR(pi1Trk->fPlab);
		  fdeltaR = dR<fdeltaR ? dR : fdeltaR;
		  dR =  fMomentum.DeltaR(pi2TrkCHANGE->fPlab);
		  fdeltaR = dR<fdeltaR ? dR : fdeltaR;
		  ...
		  dR =  muTrk->fPlab.DeltaR(pi2TrkCHANGE->fPlab);
		  fdeltaRMu = dR<fdeltaRMu ? dR : fdeltaRMu;
		  ...
		  if(pt_tmp<ptmin) ptmin=pt_tmp;
		  if(eta_tmp>etamax) etamax=eta_tmp;
		  pt_tmp=pi2TrkCHANGE->fPlab.Perp();
		  eta_tmp=fabs(pi2TrkCHANGE->fPlab.Eta());
		  if(pt_tmp<ptmin) ptmin=pt_tmp;
		  if(eta_tmp>etamax) etamax=eta_tmp;
...
int d0Reader::checkTruth(TAnaCand *cand, int truth_type)
{
	TAnaTrack *sgTrack;
	TAnaTrack *recTrack;
...
int d0Reader::CheckGoodRun(int run){
...
void d0Reader::readCuts(TString filename, int dump) {
  dump=1;
  char  buffer[200];
  fCutFile = filename;
  if (dump) cout << "==> d0Reader: Reading " << fCutFile.Data() << " for cut settings" << endl;
  ...
  if (dump) {
    cout << "====================================" << endl;
    cout << "==> d0Reader: Cut file  " << fCutFile.Data() << endl;
    cout << "------------------------------------" << endl;
  }
